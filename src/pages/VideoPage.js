...
class VideoPage extends Component {
	...
	componentDidMount = () => {
		...
		fetch(`http://localhost:8080/api/videos/${videoId}`||`https://soundtube.herokuapp.com/api/videos/${videoId}`)
		  .then(res => res.json())
		  .then(video => {
		  	this.setState({ video })
		  })
	...
	createComment = (e) => {
		e.preventDefault();
  		let videoId = this.props.match.params.video_id;
		fetch(`http://localhost:8080/api/videos/${videoId}/comments`||`https://soundtube.herokuapp.com/api/videos/${videoId}/comments`, {
			method: 'POST',
			headers: {
		    'Accept': 'application/json',
		    'Content-Type': 'application/json',
